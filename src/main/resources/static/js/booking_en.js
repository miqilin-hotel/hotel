...
function login () {
    ...
    if (isNull($('#account').val())) {
        showError($('#account'), 'Username can not be empty！')
        return;
    }
...
function querys() {
	...
	if(f.checkInDate.value<gGetToday()){
		alert('Check-in date prior to today！');
	...
	if(f.checkInDate.value>=f.checkOutDate.value){
		alert('Check-in date prior to check-in date！');
		return false;
	}
	if(f.checkOutDate.value>gDateAdd(f.checkInDate.value,1,0,0)){
		alert('Check in time is greater than one year！');
...
function sendRegisterVerifyCode() {
    ...
    if (isNull($('#registerPhone').val())) {
        showError($('#registerPhone'), 'Please enter your phone number！');
        return;
    }

    if (!checkPhone($('#registerPhone').val())) {
        showError($('#registerPhone'), 'The phone number format is wrong！');
        return;
    }

    if (isNull($('#regVerifyImgCode').val())) {
        showError($('#regVerifyImgCode'), 'Please enter a graphic verification code!');
        return;
    }
    // 发送验证码
...
function sendForgetVerifyCode() {
    ...
    if (isNull($('#forgetPhone').val())) {
        showError($('#forgetPhone'), 'Please enter your phone number！');
        return;
    }

    if (!checkPhone($('#forgetPhone').val())) {
        showError($('#forgetPhone'), 'The phone number format is wrong！');
        return;
    }
    if (isNull($('#forVerifyImgCode').val())) {
        showError($('#forVerifyImgCode'), 'Please enter a graphic verification code!');
        return;
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            countDown(obj, 30, function (showObj) {
                ...
                showObj.html('Send Verify Code');
                stopSendCode && stopSendCode();
            }, 'Has been sent');
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            showError(captchaObj, 'Failed to send');
        }
    });
}
...
function checkForm(key) {
    if (isNull($('#' + key + 'Phone').val())) {
        showError($('#' + key + 'Phone'), 'Please enter your phone number!')
        return false;
    }
    if (!checkPhone($('#' + key + 'Phone').val())) {
        showError($('#' + key + 'Phone'), 'The phone number format is wrong!')
        return false;
    }
    if (isNull($('#' + key + 'Iverifycode').val())) {
        showError($('#' + key + 'Iverifycode'), 'Please enter verification code!')
        return false;
    }
    if (!$('#' + key + 'Password').val().match(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,16}$/)) {
        showError($('#' + key + 'Password'), 'Password must be at least 8 digits and contain both upper and lower letters and numbers!')
        return false;
    }
    if ($('#' + key + 'Password').val() != $('#' + key + 'NewPassword').val()) {
        showError($('#' + key + 'Password'), '');
        showError($('#' + key + 'NewPassword'), 'Use 8 or more characters with a mix of letters, numbers & symbols!')
        return false;
    }
    return true;
}
            ...
            var startElem = {
                ...
                trigger: 'click',
                    ...
                    if (!isNull(date)) {
                        ...
                        var endHtml = '<label>Departure Time:</label><input type="text" class="input-times" value="' + endTime + '" id="calendar1" size="16" name="checkOutDate" data-date-autoclose="true" data-date-language="en" data-date-rtl="false" data-date-format="yyyy-mm-dd" data-min-view="2" data-provide="datetimepicker-inline"/>';
